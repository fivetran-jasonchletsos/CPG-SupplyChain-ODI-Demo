 ...
 * Ported from Healthcare-EPIC-Snowflake-Demo for Cardinal Provisions CPG analytics.
 *
 * Components:
...
// ─────────────────────────────────────────────────────────────────────────
// Accent tokens — aligned with Cardinal Provisions palette
// ─────────────────────────────────────────────────────────────────────────
...
export function LineagePanel({
  ...
  return (
    <div className="surface flex flex-col" style={{ minHeight: 220 }}>
      <header className="px-5 py-3 border-b flex items-center justify-between" style={{ borderColor: 'var(--hairline)' }}>
        <div className="flex items-center gap-3 flex-wrap min-w-0">
          ...
          <span className="layer-chip gold" style={{ fontSize: 10, padding: '3px 8px', fontWeight: 700 }}>
            iceberg-resolved
          </span>
        </div>
        <span className="num" style={{ color: 'var(--ink-soft)', fontSize: 12 }}>
      ...
      <div className="flex-1 relative" style={{ minHeight: 180, padding: '14px 14px 12px' }}>
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full" role="img" aria-label="Live model lineage">
          ...
          {nodes.map((n) => {
            ...
            const stroke = n.kind === 'silver' ? '#94a3b8' : (currentStep >= 6 || complete) ? C.green : C.teal;
            return (
              <g key={n.id} opacity={nodeOpacity(n)} style={{ transition: 'opacity 700ms ease' }}>
                ...
                <text
                  ...
                  style={{ fontSize: 2.6, fontFamily: '"DM Mono", monospace', fontWeight: 700 }}
                >
                  {truncate(n.label, 22)}
                </text>
              </g>
          ...
          <text x="2" y="12" fill={C.inkDim} style={{ fontSize: 2.6, letterSpacing: '0.2em', fontFamily: '"DM Mono", monospace' }}>SILVER</text>
          <text x="98" y="12" textAnchor="end" fill={C.inkDim} style={{ fontSize: 2.6, letterSpacing: '0.2em', fontFamily: '"DM Mono", monospace' }}>GOLD</text>
        </svg>
        <div className="absolute left-3 bottom-2 right-3 flex items-center justify-between num" style={{ fontSize: 11, color: 'var(--ink-muted)' }}>
          <span>{stepCaption(currentStep, complete)}</span>
          <span style={{ color: complete ? C.green : C.teal }}>
...
export function BuildCompleteSummary({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-3">
      {panels.map((p) => (
        <div key={p.title} className="surface p-4" style={{ borderTop: `3px solid ${p.accent}` }}>
          <div className="num uppercase tracking-[0.18em]" style={{ fontSize: 10, color: p.accent }}>
            {p.title}
          </div>
          <div className="mt-3 space-y-2">
            {p.stats.map((s) => (
              <div key={s.label}>
                <div className="num" style={{ fontSize: 9.5, color: 'var(--ink-soft)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
                  {s.label}
                </div>
                <div className="font-serif font-semibold" style={{ fontSize: 16, color: 'var(--ink-strong)', lineHeight: 1.2 }}>
                  {s.value}
                  {s.sub ? <span className="num" style={{ fontSize: 11, color: 'var(--ink-muted)' }}>{s.sub}</span> : null}
                </div>
              </div>
            ))}
          </div>
