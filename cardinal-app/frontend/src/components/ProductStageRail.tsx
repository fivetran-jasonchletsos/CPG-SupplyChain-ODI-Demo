...
// left to right. Ported from Healthcare-EPIC-Snowflake-Demo (Clarity / Verity /
// Altavest all share this pattern) — same four stages, Cardinal styling.
...
export default function ProductStageRail({ accent = '#0d6e6e' }: { accent?: string }) {
  return (
    <div className="mb-5 flex flex-wrap items-stretch gap-2" role="list" aria-label="Fivetran product pipeline stages">
      {STAGES.map((s, i) => (
        <div key={s.label} className="flex items-center gap-2" role="listitem">
          <div className="rounded-sm border px-3 py-2" style={{ borderColor: 'var(--hairline)', background: 'var(--cream-deep)' }}>
