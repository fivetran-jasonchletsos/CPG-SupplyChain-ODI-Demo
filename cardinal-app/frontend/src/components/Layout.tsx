...
// Cardinal nav, mirrors Clarity / Verity / Altavest:
//   - Persona links (flat) for Home + industry pages
//   - dbt-Wizard ▾ — narrative dropdown (Scenario / Live / Outcome)
//   - ODI ▾ — plumbing dropdown (Architecture / Pipeline / About)
type NavEntry =
...
const NAV: NavEntry[] = [
  ...
  { kind: 'link', to: '/retailers', label: 'Retailer OTIF' },
  { kind: 'link', to: '/demand',    label: 'Demand Sensing' },
  { kind: 'link', to: '/trade',     label: 'Trade Promotion' },
  { kind: 'link', to: '/plants',    label: 'Plants and Co-pack' },
  ...
  { kind: 'link', to: '/policy',    label: 'Policy Brief' },
  {
    ...
    label: 'dbt-Wizard',
    rootTo: '/wizard',
    matchPrefixes: ['/wizard', '/wizard-live', '/wizard-outcome'],
    children: [
      { to: '/wizard',         label: 'Scenario' },
...
// Flattened version for the mobile grid (groups become rows of links).
const NAV_FLAT: { to: string; label: string }[] = NAV.flatMap((e) =>
  e.kind === 'link' ? [{ to: e.to, label: e.label }] : e.children,
);

// ─── NavEntryEl — dark-theme variant (crimson on graphite) ──────────────────
  ...
  if (entry.kind === 'link') {
    return (
      <NavLink
        ...
        className={({ isActive }) =>
          `relative px-2.5 py-2 font-medium tracking-tight transition-colors text-[12px] whitespace-nowrap ${
            isActive ? 'text-[#ffc7cc]' : 'text-white/85 hover:text-white'
          }`
        }
      >
        {({ isActive }) => (
          <>
            {entry.label}
            {isActive && (
              <span className="absolute left-2.5 right-2.5 -bottom-[1px] h-[2px]" style={{ background: 'var(--crimson)' }} />
            )}
          </>
        )}
      </NavLink>
  ...
  return (
    <span ref={ref} className="relative inline-block">
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        className={`relative px-2.5 py-2 font-medium tracking-tight transition-colors text-[12px] whitespace-nowrap inline-flex items-center gap-1 ${
          isActive ? 'text-[#ffc7cc]' : 'text-white/85 hover:text-white'
        }`}
      >
        {entry.label}
        ...
        {isActive && (
          <span className="absolute left-2.5 right-5 -bottom-[1px] h-[2px]" style={{ background: 'var(--crimson)' }} />
        )}
      </button>
      {open && (
        <span role="menu" className="absolute left-0 top-full mt-1 min-w-[200px] rounded-sm border border-white/15 bg-[var(--graphite)] shadow-xl overflow-hidden z-50">
...
const DEMOS = [
  ...
  { key: 'supplychain',     name: 'Manifest',             industry: 'Supply chain, 3PL logistics',              url: 'https://fivetran-jasonchletsos.github.io/SupplyChain-ODI-Demo/',               accent: '#0891b2' },
  ...
  { key: 'cpg',             name: 'Cardinal Provisions',  industry: 'CPG, Supply chain and demand',             url: 'https://fivetran-jasonchletsos.github.io/CPG-SupplyChain-ODI-Demo/',           accent: '#b1182b' },
...
export default function Layout() {
  ...
  return (
    <div className="min-h-full flex flex-col bg-[var(--cream)]">
      <div className="cardinal-rail" />

      <header className="bg-[var(--graphite)] text-white sticky top-0 z-30">
        <div className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8">
          <div className="flex h-16 sm:h-20 items-center justify-between gap-2 sm:gap-6">
            <Link to="/" className="flex items-center gap-3 shrink-0 min-w-0 group">
              <div className="h-10 w-10 rounded-sm flex items-center justify-center" style={{ background: 'var(--crimson)' }}>
                <CardinalMark className="h-6 w-6 text-white" />
              </div>
              <div className="leading-tight min-w-0">
                <div className="font-serif font-semibold text-lg sm:text-xl tracking-tight truncate">
                  Cardinal Provisions
                </div>
                <div className="mt-0.5 text-[10px] sm:text-[11px] font-medium uppercase tracking-[0.18em]" style={{ color: '#ffc7cc' }}>
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <button
                ...
                className="xl:hidden h-9 w-9 inline-flex items-center justify-center rounded-sm text-white/90 hover:bg-white/10"
              >
          ...
          {mobileOpen && (
            <div className="xl:hidden pb-4 border-t border-white/10 pt-3 space-y-3">
      ...
      <footer className="border-t border-[var(--hairline)] bg-[var(--graphite)] text-white/85 mt-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 grid grid-cols-1 md:grid-cols-3 gap-8 text-sm">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <div className="h-7 w-7 rounded-sm flex items-center justify-center" style={{ background: 'var(--crimson)' }}>
                <CardinalMark className="h-4 w-4 text-white" />
              </div>
              <div className="font-serif font-semibold text-white">Cardinal Provisions</div>
            </div>
            <p className="leading-relaxed text-white/65">
              Supply-chain control tower built on Fivetran Open Data Infrastructure.
  ...
  return (
    <div ref={wrapRef} className="relative hidden sm:block">
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        title="Switch demo"
        className="inline-flex items-center gap-1.5 rounded-sm px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider border bg-[var(--crimson)]/20 text-[#ffc7cc] border-[var(--crimson)]/40 hover:bg-[var(--crimson)]/30"
      >
        <span className="h-1.5 w-1.5 rounded-full bg-[#ffc7cc] animate-pulse" />
        Snapshot
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-[300px] rounded-sm border border-[var(--hairline)] bg-white shadow-xl z-40 overflow-hidden">
          ...
          <div className="py-1 max-h-[440px] overflow-y-auto">
            {DEMOS.map((d) => {
...
function CardinalMark({ className = '' }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={className} aria-hidden>
      <path d="M12 3 L19 16 L5 16 Z" fill="currentColor" stroke="none" />
