/*
 * WizardLivePage — dbt-wizard live-build playback for Cardinal Provisions.
 *
...
const STEP_DEFS = [
  ...
  { label: 'Schema Understanding', who: 'Summary',      tools: 'describe, lineage',       insight: '4 cols — 0 null keys'  },
...
export default function WizardLivePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(wizardDataUrl('wizard_agents.json')).then(r => { if (!r.ok) throw new Error(`Failed to fetch agents: ${r.status}`); return r.json(); }),
      fetch(wizardDataUrl('wizard_scenario.json')).then(r => { if (!r.ok) throw new Error(`Failed to fetch scenario: ${r.status}`); return r.json(); }),
      fetch(wizardDataUrl('wizard_build_script.json')).then(r => { if (!r.ok) throw new Error(`Failed to fetch build script: ${r.status}`); return r.json(); }),
    ]).then(([a, s, b]) => {
      setAgents(a.agents);
      setScenario(s);
      setEvents(b.events);
  ...
  if (!scenario || agents.length === 0 || events.length === 0) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-12">
        ...
        <p className="num text-sm mt-2" style={{ color: 'var(--ink-muted)' }}>Loading build playback...</p>
      </div>
    );
  }
    ...
    <div className="wizard-terminal mx-auto max-w-[1640px] px-4 py-4 sm:px-6 lg:px-8">
      ...
      <div
        ...
        style={{
          ...
          borderLeft: '4px solid var(--teal, #0d6e6e)',
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <span
            ...
            style={{
              ...
              background: 'rgba(13,110,110,0.1)', color: 'var(--teal, #0d6e6e)', border: '1px solid rgba(13,110,110,0.35)',
            }}
          >
            <span
              style={{
          ...
          <span className="num" style={{ color: 'var(--ink-muted)', fontSize: 13 }}>
            Step{' '}
            <span style={{ color: 'var(--teal, #0d6e6e)', fontWeight: 700 }}>{currentStep}/{totalSteps}</span>
          ...
          >
            <div
              style={{
                ...
                height: '100%',
                background: complete ? 'var(--forest)' : 'var(--teal, #0d6e6e)',
      ...
      <div
        className="mb-3 px-4 py-2.5 surface border-l-4 flex items-center gap-5 flex-wrap"
        style={{ borderLeftColor: 'var(--crimson)' }}
      >
        <div className="min-w-0 flex-shrink" style={{ flex: '1 1 460px' }}>
          <div className="eyebrow" style={{ fontSize: 10, marginBottom: 2 }}>
            Demand Planning — {scenario.timezone_label} — {scenario.requested_by}
          </div>
          <p
        ...
        <div className="num shrink-0" style={{ fontSize: 11, color: 'var(--ink-muted)' }}>
          Target: <span style={{ color: 'var(--teal, #0d6e6e)', fontWeight: 700 }}>{scenario.metric_code}</span>
        </div>
      </div>
            ...
            >
              <div
                ...
                style={{
                  ...
                  color: active ? 'var(--teal, #0d6e6e)' : done ? 'var(--forest)' : 'var(--ink-soft)',
                }}
              >
              ...
              <div className="font-semibold truncate" style={{ fontSize: 13, lineHeight: 1.15, color: 'var(--ink-strong)' }}>
                {s.label}
              </div>
              <div
      ...
      >
        ...
        <section
          className="surface flex flex-col lg:!h-[calc(100dvh-440px)]"
        ...
        >
          ...
          >
            <div>
              ...
              <div className="num mt-0.5" style={{ fontSize: 12, color: 'var(--ink-muted)' }}>
          ...
          >
            {visibleNarr.map((m, idx) => {
              ...
              const color = a?.color ?? AGENT_STEP_COLOR[m.e.from] ?? '#0d6e6e';
                      ...
                      <div className="flex items-center gap-2 flex-wrap mb-1.5">
                        ...
                        <span
                          ...
                          style={{
                            ...
                            background: 'rgba(13,110,110,0.10)', color: 'var(--teal, #0d6e6e)',
        ...
        <section className="flex flex-col gap-3 lg:!h-[calc(100dvh-440px)]" style={{ minHeight: 'max(60vh, 300px)' }}>
          ...
          {/* SQL panel */}
          <div className="surface flex flex-col" style={{ flex: '1.7 1 0' }}>
            <header
              className="px-5 py-3 border-b flex items-center justify-between"
              style={{ borderColor: 'var(--hairline)' }}
            >
              <div className="flex items-center gap-3 flex-wrap min-w-0">
                <div className="eyebrow num" style={{ fontSize: 11, letterSpacing: '0.02em' }}>
                  models/gold/fct_lift_by_sku_promo_region_week.sql
                </div>
                <span
                  className="layer-chip"
                  style={{
              ...
              <span className="num" style={{ fontSize: 12, whiteSpace: 'nowrap', color: 'var(--ink-soft)' }}>
          ...
          {/* YAML panel */}
          <div className="surface flex flex-col" style={{ flex: '1 1 0' }}>
            <header
              className="px-5 py-3 border-b flex items-center justify-between"
              style={{ borderColor: 'var(--hairline)' }}
            >
              <div className="flex items-center gap-3 flex-wrap min-w-0">
                <div className="eyebrow num" style={{ fontSize: 11, letterSpacing: '0.02em' }}>
                  models/gold/fct_lift_by_sku_promo_region_week.yml
                </div>
                <span
                  className="layer-chip"
                  style={{
                    color: '#14532d', background: 'rgba(20,83,45,0.07)',
      ...
      <div className="surface mt-2 px-3 py-2 flex items-center gap-3">
        <div className="eyebrow shrink-0" style={{ fontSize: 10 }}>tool calls</div>
        {state.sideEffects.length === 0 ? (
          <div className="num" style={{ fontSize: 11.5, color: 'var(--ink-soft)' }}>Awaiting first tool call...</div>
        ) : (
          <ul className="flex items-center gap-x-4 gap-y-1 flex-wrap min-w-0">
            {state.sideEffects.slice(0, 4).map((s, i) => (
              <li
                key={`${s}-${i}`}
                className="flex items-center gap-1.5 num truncate"
                style={{ fontSize: 11.5, maxWidth: '32ch', color: 'var(--ink)' }}
                title={s}
              >
                <span
                  style={{
                    display: 'inline-block', width: 7, height: 7, borderRadius: 999, flexShrink: 0,
                    background: i === 0 ? 'var(--teal, #0d6e6e)' : 'var(--ink-soft)',
            ...
            <div className="flex items-baseline gap-3 flex-wrap">
              <div
                ...
                style={{
                  ...
                  background: 'rgba(20,83,45,0.12)', color: 'var(--forest)',
      ...
      <style>{`
        ...
        .wizard-terminal::after {
          content: 'cardinal-provisions/wizard-live — dbt-wizard';
          position: absolute;
          top: 0; left: 0; right: 0;
          height: 28px;
          display: flex;
        ...
        .wizard-terminal .surface > div[style*="background: var(--paper)"] {
        ...
        .wizard-terminal [style*="color: var(--teal"] { color: var(--t-accent) !important; }
        ...
        .wizard-terminal .surface[style*="rgba(20,83,45"] {
          background: rgba(74,222,128,0.10) !important;
        }
        .wizard-terminal .surface[style*="var(--cream)"] {
          background: var(--t-surface) !important;
        }
        .wizard-terminal pre {
          background: var(--t-bg) !important;
...
const SQL_KEYWORDS = new Set([
  ...
  'sum', 'max', 'min', 'avg', 'dateadd', 'datediff', 'date_trunc', 'current_date',
  'is', 'not',
]);
